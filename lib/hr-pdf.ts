...
// A dark (ink + blue) version of the wordmark for light backgrounds like the
// certificate, so no coloured chip is needed behind it.
let logoDarkBuffer: Buffer | null | undefined;
function getLogoDark(): Buffer | null {
  if (logoDarkBuffer !== undefined) return logoDarkBuffer;
  try {
    logoDarkBuffer = fs.readFileSync(path.join(process.cwd(), "public", "logo-dark.png"));
  } catch {
    logoDarkBuffer = null;
  }
...
function buildCertificatePdf(doc: CertificateDoc): Promise<Buffer> {
  return new Promise((resolve) => {
    ...
    // Header — the dark wordmark sits directly on the cream (clean, no chip).
    let y = 48;
    const logo = getLogoDark() || getLogo();
    let logoDrawn = false;
    if (logo) {
      try {
        pdf.image(logo, cx - 78, y, { width: 156 });
        y += 58;
        logoDrawn = true;
      } catch {
        logoDrawn = false;
    ...
    if (!logoDrawn) {
      pdf.font("Times-Bold").fontSize(22).fillColor(INK).text(co.name, 0, y, { width: pageW, align: "center" });
      y += 30;
    }
